...
> 📌 **남은 작업(할 일) → 노션 칸반 "클루드 프로젝트 테스크"** (단일 원천 — HANDOVER는 할 일 목록이 아님)
> 📌 **현재상태·시트/배포 ID·트랙별 배경 상세 → [`HANDOVER.md`](HANDOVER.md)** (이 폴더, PWA 단일 핸드오버)
> 📌 **완료 작업 이력 → 노션 칸반의 완료 카드** (2026-07-27 일원화. `../WORKLOG.md`는 아카이브)
> 이전에 흩어져 있던 분산 핸드오버 3종은 2026-06-17 `HANDOVER.md`로 통합·삭제됨.
...
- **AI가 직접 처리 우선**. 사용자에게 복붙·재배포 같은 반복 작업 떠넘기지 말 것. 자동화 막히면 도구/플러그인 설치 제안 (예: clasp으로 GAS 자동 푸시·배포 — 현재 적용 중)
- **추측 금지**. 모르겠으면 추정하지 말고 되묻기. 모르면 "모른다" 인정. 그 다음 curl / GAS 로그 / 시트 publish CSV로 검증
- **묻는 말에 답하기**. 질문을 질문으로 되돌리지 말 것
- **시트 컬럼명 그대로**. 자의로 다른 이름 만들지 말 것 — 시트가 단일 진실
- **다중 시트 컬럼명 불일치 시 사용자에게 알리고 승인 후 수정**. 같은 의미인데 시트마다 다른 이름이면 임의 통일 금지
- **명시적 중단 지시 존중**. "아무런 액션 하지마" 같은 지시 받으면 즉시 멈춤
...
- **2026-06-15 보강 (소스 경계)**: 소스별 dedup만으론 2025↔2026 경계 계약(2025 주문이 2026에 차수 올라 마무리 → 같은 키가 시트+API 양쪽에 잔존)이 안 잡혀 이중계상됨. → `loadAllProcurementData`가 concat 후 `pickFinalRevisionPerContract`를 **1회 더** 적용(`[통합]`). 발견 당시 136건/-57.3억 제거. 커밋 38b1fba
